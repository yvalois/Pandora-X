...
export default function ModalTP() {
  ...
  const { frenchiesMinter, accountAddress } = useSelector(
  ...
  const close = () => {
    closeModal();
    window.localStorage.removeItem('TransferFId');
  };

  const verifyApprove = async () => {
    const isap = await frenchiesMinter.getApproved(id); //MarketPlace
  ...
  const Approve = async () => {
    if (validator.isEthereumAddress(value)) {
      setLoading(true);
      setError('');
      const tx = await frenchiesMinter.approve(value, id);
  ...
  const transfer = async () => {
    setLoading(true);
    const tx = await frenchiesMinter.transferFrom(accountAddress, value, id);
    await tx.wait();
  ...
  useEffect(() => {
    const _id = window.localStorage.getItem('TransferFId');
    setId(_id);
  }, []);
