...
export default function ModalTP() {
  ...
  const { productoMinter, accountAddress } = useSelector(
    (state) => state.blockchain
  );
  const dispatch = useDispatch<AppDispatch>();

  const close = () => {
    closeModal('TRANSFER_P');
    window.localStorage.removeItem('TransferPId');
  };

  const verifyApprove = async () => {
    const isap = await productoMinter.getApproved(id); //MarketPlace
  ...
  const Approve = async () => {
    if (validator.isEthereumAddress(value)) {
      setLoading(true);
      setError('');
      const tx = await productoMinter.approve(value, id);
  ...
  useEffect(() => {
    const _id = window.localStorage.getItem('TransferPId');
    setId(_id);
  }, []);
